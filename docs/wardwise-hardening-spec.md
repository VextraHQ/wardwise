...
| `src/features/candidates/schemas/{candidate,canvasser}-schemas.ts`, `src/features/collect/schemas/collect-schemas.ts`, `src/lib/schemas/admin-schemas.ts` | Feature-owned Zod schemas; admin self-service profile/email/password stays in `admin-schemas.ts`   |
