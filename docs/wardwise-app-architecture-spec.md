...
| Phase 7 - Candidate Dashboard Relocation     | Next        | Move `src/components/candidate-dashboard/*`, `use-candidate-dashboard`, `lib/api/candidate-dashboard.ts`, and `src/lib/candidate/analytics.ts`. Watch for the Phase 4 fixup note: `features/candidates/lib/directory.ts` still imports `analytics.ts` — pick a neutral home or split before completing the move.                                                                                                                                                                                                                          |
...
13. Start Phase 7 (Candidate dashboard relocation). Move `src/components/candidate-dashboard/*`, `use-candidate-dashboard`, `lib/api/candidate-dashboard.ts`, and `src/lib/candidate/analytics.ts`. Resolve the `features/candidates/lib/directory.ts → analytics.ts` cross-feature dependency before completing the move.
