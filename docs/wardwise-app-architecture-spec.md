...
| Phase 5 - Geo Relocation                     | Complete    | Admin geo components, hook, lib (incl. `constituency-server` → `server/`), api (renamed `{geo,location}-api.ts`), schemas, and 6 geo data files (`nigerian-{constituencies,federal-constituencies,senatorial-districts}`, `state-lga-locations`, `wards`, `polling-units`) now under `src/features/geo/`. `nigerian-parties.ts` stays in `src/lib/data/` (not geo). Bundle composition preserved — file moves only, no barrel re-exports introduced. |
| Phase 6 - Reporting Relocation               | Next        | Collect-reporting already resolved to `features/collect` in Phase 3; Phase 6 focuses on `campaign-report` components, `use-campaign-report`, `use-campaign-insights-scope`, `campaign-report-api`, `report-access`, and `campaign-report.ts` types.                                                                                                                                                                                                  |
...
11. ~~Start Phase 5 (Geo relocation). Move `src/components/admin/geo/*`, `use-geo`, `lib/geo/*`, geo API clients, schemas, and data files. Watch bundle impact for static geo data.~~ (done — all geo files now under `src/features/geo/`; `nigerian-parties.ts` correctly left in `src/lib/data/` as non-geo party reference; CLI scripts and prisma seed files updated to new paths.)
12. Start Phase 6 (Reporting relocation). Move `src/components/campaign-report/*`, `use-campaign-report`, `use-campaign-insights-scope`, `lib/api/campaign-report.ts`, `lib/server/report-access.ts`, `src/types/campaign-report.ts` into `features/reporting/`.
