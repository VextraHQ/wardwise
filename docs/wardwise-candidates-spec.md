...
> Branch: `main` | Last updated: 2026-05-04
...
Deletes candidate and associated user account. Requires a JSON body with
`{ confirmationEmail }`; the normalized value must match the candidate account
email before the cascade delete is allowed.
...
- **Account status pill bar** above the toolbar: All / Pending / Credentials Sent / Active / Suspended (with live counts) — primary navigation axis, mirrors the Collect status tabs
- **Secondary filters** (desktop row + mobile sheet, in `CandidateFilters`):
  - **Collect** (engagement) — Any engagement · Has active campaign · Has campaign (inactive) · No campaign yet
  - **Insights** (engagement) — All insights · Insights on · Insights off
  - **Position**, **Party** — demographic single-selects
  - **Sort** — Newest first · Name (A–Z) · Most campaigns · Most supporters
- Engagement filters (Collect/Insights) use the API-derived primary `collectCampaign` payload — no schema change needed.
...
- `StepProgress` is **clickable for back-jumps**: completed segments become `<button>`s that call `onStepClick(index)`. Forward jumps still go through Continue so per-step validation runs. Discoverability is built into the component itself (not the orchestrator) so every consumer gets the same affordance:
  ...
  - Adopted across every `StepProgress` consumer so the interaction is consistent app-wide: the candidate creation wizard, the admin campaign wizard (`features/collect/components/admin/wizard/campaign-wizard.tsx`), and the public canvasser/supporter collect flow (`collect/campaign-registration-form.tsx`).
...
- **Draft autosave** is owned by the generic `useWizardDraft` hook (`src/hooks/shared/use-wizard-draft.ts`). The orchestrator passes the RHF instance, current step, the storage key (`wardwise:create-candidate:draft:v1`), an `isMeaningful` predicate, and `defaultValues`. The hook handles debounced (400ms) writes, 24h TTL expiry, restore-on-mount, and exposes both `discard()` (clears storage **and** resets the form) and `clear()` (storage-only — used after a successful submit so the credentials dialog isn't preceded by a flash of empty form). The hook is **not** candidate-named on purpose: it is schema-agnostic RHF + step persistence only; **candidate** is expressed by the storage key and predicates in `create-candidate-form.tsx`. The **campaign wizard** (`features/collect/components/admin/wizard/campaign-wizard.tsx`) is the second consumer (`wardwise:campaign-wizard:draft:v2` + `isMeaningfulCampaignDraft`). Further admin wizards can reuse the same hook with their own keys. **Parallel in Collect:** `useCollectFormPersistence` (`src/features/collect/hooks/use-collect-form-persistence.ts`) does the same _kind_ of job (localStorage, debounced saves, restore) but is Collect-specific — it keys off `campaignSlug`, persists `screen` + extra `uiState`, and coordinates geo restore refs. The naming split (`wizard` vs `collect-form`) is intentional so neither hook pretends to be the other’s generalization.
...
- Danger zone: Inline reviewed delete flow. The first action is a non-filled `Review Deletion` button; expanding the section shows the deletion impact summary (login account, campaign count, submission count, canvasser records, and affected `/c/[slug]` links). The final destructive button stays disabled until the admin types the candidate account email. The same confirmation email is required by the DELETE API, so the safety check is enforced server-side as well as in the UI.
...
| `src/features/candidates/components/candidate-overview.tsx`               | Overview tab orchestrator (form state, saves, composes stats + profile card)                                                                 |
| `src/features/candidates/components/overview/candidate-overview-profile-card.tsx` | Profile card (summary + identity/electoral/contact/bio sections)                                                                   |
| `src/features/candidates/lib/candidate-overview-helpers.ts`               | Overview defaults, `EditingSection`, shared field helpers                                                                                    |
| `src/features/candidates/hooks/use-candidate-directory.ts`                | Candidate directory filters, sort, pagination state                                                                                          |
| `src/features/candidates/components/candidate-directory-badges.tsx`       | Directory row badges + actions menu                                                                                                          |
| `src/features/candidates/components/candidate-directory-records.tsx`      | Directory mobile cards + desktop table                                                                                                       |
...
| `src/features/candidates/components/candidate-management.tsx`             | Directory page shell (header, status bar, filters toolbar)                                                                                   |
...
| Date       | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-05-04 | Candidate delete safety upgraded from a simple confirmation dialog to an inline reviewed danger-zone flow on the Account tab. The expanded flow shows the cascade impact, affected campaign slugs, and requires typing the candidate account email before enabling the final destructive action. `DELETE /api/admin/candidates/[id]` now requires `confirmationEmail` and rejects mismatches before deleting, with richer audit details for the cascade.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
