...
//   // For mock: Checks both mock voters array AND localStorage for incomplete registrations
...
//     // First, check mock voters array (existing voters)
//     const voter = getVoterByNIN(nin);
//     if (voter) {
//       const status = computeRegistrationStatus(voter.lastCompletedStep);
//       return {
//         exists: true,
//         status,
//         voter,
...
//     // Second, check registration store localStorage for incomplete registrations
//     // Zustand already persists the registration store, so we can check it directly
...
//           const hasBasicData =
//             !!registrationData?.state?.payload?.basic?.firstName;
//           const hasLocation =
//             !!registrationData?.state?.payload?.location?.state;

//           // If NIN matches and has some data but not all, it's incomplete
//           if (storedNIN === nin && (hasBasicData || hasLocation)) {
//             const payload = registrationData.state.payload;
//             const incompleteVoter: Voter = {
//               id: `incomplete-${nin}`,
//               nin: payload.nin || nin,
...
//               lastCompletedStep: storedStep || "nin",
